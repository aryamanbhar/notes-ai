                ...
                if key in st.session_state["selected_annots"] and key in st.session_state["ai_outputs"]:
                    ...
                    if "error" in outputs:
                        ...
                        with st.expander("AI Suggestions"):
                            ...
                            edit_el5 = st.text_area("Edit ELI5", outputs['el5'], key=f"el5_{key}")
                            st.markdown(f"**Mnemonic**: {outputs['mnemonic']}")
                            edit_mnemonic = st.text_area("Edit Mnemonic", outputs['mnemonic'], key=f"mnemonic_{key}")
                            st.markdown(f"**Analogy**: {outputs['analogy']}")
                            edit_analogy = st.text_area("Edit Analogy", outputs['analogy'], key=f"analogy_{key}")
                            if st.button("Save Edits", key=f"save_{key}"):
                                st.session_state["ai_outputs"][key]["el5"] = edit_el5
                                st.session_state["ai_outputs"][key]["mnemonic"] = edit_mnemonic
                                st.session_state["ai_outputs"][key]["analogy"] = edit_analogy
...
# Footer
st.markdown("---")
st.caption("Powered by xAI Grok-3.1-8b-128k via Hugging Face | Built with Streamlit")